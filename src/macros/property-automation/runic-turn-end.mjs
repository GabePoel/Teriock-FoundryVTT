if (
  ...
  const content = await TextEditor.implementation.enrichHTML(teriock.helpers.string.dedent(`
  ...
  const proceed = await teriock.applications.api.TeriockDialog.confirm({
    ...
    yes: {
      ...
      label: _loc("TERIOCK.ROLLS.Base.name", { value: _loc("TERIOCK.SYSTEMS.Equipment.MENU.repair") }),
    },
  });
  if (proceed) {
    const roll = new teriock.dice.rolls.BaseRoll("2d4kh1", {}, {
      flavor: _loc("TERIOCK.ROLLS.Base.name", { value: _loc("TERIOCK.SYSTEMS.Equipment.MENU.repair") }),
      threshold: 4,
    });
    await roll.evaluate();
    await roll.toMessage({ speaker: ChatMessage.getSpeaker({ actor: scope.item.actor }) }, {
      rollMode: game.settings.get("core", "rollMode"),
    });
    if (roll.total === 4) await scope.item.system.repair();
  }
}

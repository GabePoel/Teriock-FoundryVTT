...
await tm.utils.progressBar(
  ...
  async (page) => {
    ...
    let familiarItem = game.teriock.packs.powers.index.getName(name);
    ...
    const creature = game.teriock.packs.creatures.index.getName(animal);
    if (creature)
    ...
    if (!familiarItem) {
      familiarItem = await game.teriock.Item.create(
        {
          folder: game.teriock.packs.powers.folders.getName("Familiars").id,
      ...
      await familiarItem.update({ system: familiarItemSystem });
    }

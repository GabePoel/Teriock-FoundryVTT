 ...
 * @property {TeriockWrapper} document
 * @property {TeriockWrapper} item
 */
export default class WrapperSheet extends BaseItemSheet {
  ...
  async _preRender(context, options) {
    ...
    if (this.document.effects.size === 0) {
      ...
      const type = await selectDialog(typeChoices, {
        label: _loc("TERIOCK.SHEETS.Wrapper.DIALOG.label"),
        hint: _loc("TERIOCK.SHEETS.Wrapper.DIALOG.hint"),
      });
  ...
  async render(options = {}) {
    ...
    const out = await this.document.system.effect.sheet.render(options);

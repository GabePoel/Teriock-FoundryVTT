...
const command = {
  ...
  label: options => TERIOCK.config.attribute[options?.attribute]?.abbreviation
    ? _loc("TERIOCK.ROLLS.Feat.name", { value: TERIOCK.config.attribute[options?.attribute]?.abbreviation })

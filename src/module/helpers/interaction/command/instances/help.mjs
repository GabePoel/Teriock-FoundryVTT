...
export function createHelpCommand(allCommands) {
  ...
  const uniqueCommands = Object.values(allCommands)
    .filter((cmd, index, arr) => arr.findIndex((other) => other.id === cmd.id) === index);

  return new TeriockCommand("help", "List all available Teriock chat commands.", async ({ chatData }) => {
    const helpHTML = uniqueCommands
      .sort((a, b) => a.id.localeCompare(b.id))
      .map((cmd) => {
        const aliases = cmd.aliases.length > 0 ? ` <span class="aliases">(aliases: ${cmd.aliases.map((a) => `/${a}`)
          .join(", ")})</span>` : "";

    ...
    _filterDocumentCreationData(embeddedName, data) {
      if (embeddedName === "ActiveEffect") {
        for (let i = data.length - 1; i >= 0; i--) {
          ...
          if (foundry.utils.hasProperty(effectData, "type")
            && !this.metadata.childEffectTypes.includes(foundry.utils.getProperty(effectData, "type"))) {
            data.splice(i, 1);
          }
        }
        ...
        for (let i = data.length - 1; i >= 0; i--) {
          ...
          if (foundry.utils.hasProperty(itemData, "type")
            && !this.metadata.childItemTypes.includes(foundry.utils.getProperty(itemData, "type"))) {
            data.splice(i, 1);
          }
        }
      }

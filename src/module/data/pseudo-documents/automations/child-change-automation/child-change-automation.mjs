...
export default class ChildChangeAutomation extends CritMechanicMixin(BaseAutomation) {
  ...
  static defineSchema() {
    ...
    return Object.assign(super.defineSchema(), {
      ...
      key: new fields.StringField({
        choices: objectMap(TERIOCK.config.change.child.paths, e => e.label, { localize: true }),
        initial: initialKey,
        required: true,
      }),
  ...
  get _changeTypeChoices() {
    ...
    if (types) {
      return Object.fromEntries(
        Object.entries(ActiveEffect.CHANGE_TYPES).filter(([k, _v]) => types.includes(k)).map((
          [_k, v],
  ...
  /** @returns {Record<string, string>} */
  get _keyChoices() {
    return objectMap(TERIOCK.config.change.child.paths, e => e.label, { filter: e => e.targets.includes(this.target) });
  }
  ...
  prepareData() {
    ...
    if (!changeTypeChoices.includes(this.changeType)) {
      this.changeType = /** @type {Teriock.Changes.Type} */ (changeTypeChoices[0]);
    }
    const keyChoices = Object.keys(this._keyChoices);
    if (!keyChoices.includes(this.key)) {
      this.key = /** @type {Teriock.Changes.ChildPath} */ (keyChoices[0]);
    }
  }
}

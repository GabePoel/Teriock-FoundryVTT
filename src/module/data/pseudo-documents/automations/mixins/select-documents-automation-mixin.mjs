...
export default function SelectDocumentsAutomationMixin(Base) {
  ...
  class SelectDocumentsAutomation extends mixClasses(Base, SelectExternalDocumentsAutomationMixin) {
    ...
    static defineSchema() {
      return Object.assign(super.defineSchema(), {
        local: new fields.SchemaField({ identifiers: new TypedIdentifierSetField(), qualifier: qualifierField() }),
      });
    }
    ...
    async getDocuments(options = {}) {
      ...
      const local = await Promise.all(this.local.identifiers.map(identifier => fromIdentifierLocal(identifier, actor)));
  ...
  return SelectDocumentsAutomation;
}

...
export function _buildTags(rollConfig) {
  ...
  if (mpSpent > 0) {
    rollConfig.chatData.system.tags.push(`${mpSpent} MP Spent`);
  }
  const hpSpent = rollConfig.useData.costs.hp;
  if (hpSpent > 0) {
    rollConfig.chatData.system.tags.push(`${hpSpent} HP Spent`);
  }
  const gpSpent = rollConfig.useData.costs.gp;
  if (gpSpent > 0) {
    rollConfig.chatData.system.tags.push(`${gpSpent} ₲ Spent`);
  }
}

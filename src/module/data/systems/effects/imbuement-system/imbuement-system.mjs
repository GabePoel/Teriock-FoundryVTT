...
export default class ImbuementSystem extends mix(
  ...
  get _nameTags() {
    ...
    if (this.critical) {
      tags.unshift(_loc("TERIOCK.SYSTEMS.Imbuement.PANELS.critical"));
    }
    return tags;
  }
    ...
    parts.bars = [
      {
        ...
        label: _loc("TERIOCK.SYSTEMS.Imbuement.PANELS.duration"),
        wrappers: [this.parent.remainingString],
      },
      {
        icon: TERIOCK.display.icons.document.condition,
        label: _loc("TERIOCK.SYSTEMS.Imbuement.PANELS.conditions"),
        wrappers: [
          ...Array.from(
          ...
          this.critical
            ? _loc("TERIOCK.SYSTEMS.Imbuement.PANELS.critical")
            : "",
          this.heightened
            ? this.heightened === 1

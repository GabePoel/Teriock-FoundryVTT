/** @import { PseudoMetadata } from "./_types" */
 ...
 */
export default class Pseudo extends foundry.abstract.DataModel {
  /**
  ...
  get uuid() {
    ...
    while (!(parent instanceof Pseudo) && 
           !(parent instanceof foundry.abstract.Document)) {
  ...
  get fieldPath() {
    ...
    if (this.parent instanceof Pseudo) {
   ...
   * is not one-to-one.
   * @type {PseudoApplication | null}
   */
  get sheet() {
    const sheet = PseudoApplication.getSheet(this);
  ...
  prepareBaseData() {
    ...
    for (const documentName of documentNames) {
      for (const pseudo of this.getEmbeddedPseudoCollection(documentName)) {
   ...
   */
  getEmbeddedPseudoCollection(embeddedName) {
